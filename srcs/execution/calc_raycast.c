...
void calc_step_deltas(t_cub3d *cub3d, t_raycast *cast)
{
	...
	cast->delta_dist_y = sqrt(1 + (cast->ray_dir_x * cast->ray_dir_x) / \
								(cast->ray_dir_y * cast->ray_dir_y));
	if (cast->ray_dir_x < 0)
	{
		cast->step_x = -1;
...
void calc_wall_dist(t_cub3d *cub3d, t_raycast *cast)
{
	if (cast->side_hit == vertical)
	{
		...
		cast->wall_dist = (cast->map_x - cub3d->player.x + (1 - cast->step_x) / 2) / cast->ray_dir_x;
	}
	else
	{
		if (cast->step_y == -1)
		...
		cast->wall_dist = (cast->map_y - cub3d->player.y + (1 - cast->step_y) / 2) / cast->ray_dir_y;
	}
}
//end is wall botom
void calc_wall(t_cub3d *cub3d, t_raycast *cast)
{
	...
	cast->tex_step =
				cast->texture->height / (cast->wall_bottom - cast->wall_top);
